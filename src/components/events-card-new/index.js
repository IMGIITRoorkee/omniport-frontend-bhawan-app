    ...
    return (
      <div
        className={tailwindWrapper(
          `flex justify-around items-center p-2 rounded-lg ${backgroundColorClass}`
        )}
      >
        <div>
          <h3 className={tailwindWrapper("text-xl font-semibold mb-2")}>
            {eventName}
          </h3>
        </div>
        ...
        <div className={tailwindWrapper("flex flex-col gap-2")}>
          <button
            className={tailwindWrapper(
              "text-[#133BC5] bg-white border border-[#133BC5] font-bold px-4 py-2 rounded-lg"
            )}
            onClick={onRegister}
          >
            Register
